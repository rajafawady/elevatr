...
export const syncLocalDataToFirebase = async (firebaseUser: User): Promise<SyncResult> => {
  ...
  try {
    ...
    // If this is a cached user (signed out from same Firebase account), merge with cached data
    if (localUser.uid.startsWith('cached_') && localUser.originalFirebaseUid === firebaseUser.uid) {
      console.log('Detected cached user data for same Firebase account, merging changes...');
      
      // Get cached data from before sign-out
      const cachedData = restoreCachedData(firebaseUser.uid);
      if (cachedData) {
        // Merge cached data with current local changes
        const mergedData = await mergeLocalChangesWithCachedData(localData, cachedData);
        console.log('Merged local changes with cached data:', mergedData);
        
        // Sync merged data to Firebase
        await syncMergedDataToFirebase(firebaseUser, mergedData, result);
      } else {
        // No cached data found, sync local data normally
        await syncNormalLocalDataToFirebase(firebaseUser, localData, result);
      }
    } else {
      // Regular local user or different Firebase account, sync normally
      await syncNormalLocalDataToFirebase(firebaseUser, localData, result);
    }
...
export const hasLocalDataToSync = (): boolean => {
  try {
    ...
    // Check for regular local user data
    if (localUser.uid.startsWith('local_')) {
      const sprints = localStorageService.getLocalSprints(localUser.uid);
      return sprints.length > 0;
    }
    
    // Check for cached user data (from signed out authenticated user)
    if (localUser.uid.startsWith('cached_')) {
      const sprints = localStorageService.getLocalSprints(localUser.uid);
      return sprints.length > 0;
    }
...
// Helper function to merge local changes with cached data
const mergeLocalChangesWithCachedData = async (localData: any, cachedData: any) => {
  // This function should merge local changes made while signed out
  // with the cached data from before sign-out
  
  // For now, prioritize local changes over cached data
  // In a more sophisticated implementation, you could do timestamp-based merging
  return {
    sprints: [...localData.sprints], // Local changes take precedence
    progress: [...localData.progress]
  };
};

// Helper function to sync merged data to Firebase
const syncMergedDataToFirebase = async (firebaseUser: User, mergedData: any, result: SyncResult) => {
  // Sync the merged data to Firebase
  await syncNormalLocalDataToFirebase(firebaseUser, mergedData, result);
};

// Helper function to sync normal local data to Firebase (extracted from original logic)
const syncNormalLocalDataToFirebase = async (firebaseUser: User, localData: any, result: SyncResult) => {
  // Sync sprints
  for (const localSprint of localData.sprints) {
    try {
